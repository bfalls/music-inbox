...
music_inbox_install_default_request_template "$source_template" "$drafts" "$state" >/dev/null
[[ -f "$drafts/Default Music Request v2.md" ]]
...
new_source="$TEST_ROOT/Default Music Request v3.md"
sed 's/music-inbox-template-version: 2/music-inbox-template-version: 3/' "$source_template" > "$new_source"
music_inbox_install_default_request_template "$new_source" "$fresh_drafts" "$fresh_state" >/dev/null
rg -q 'music-inbox-template-version: 3' "$fresh_drafts/Default Music Request.md"
[[ ! -e "$fresh_drafts/Default Music Request v3.md" ]]
