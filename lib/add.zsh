...
music_inbox_submit_request() {
  local url="$1" mode="$2" playlist="${3:-}" create_playlist="${4:-no}" language="${5:-}" formats="${6:-}"
  local temporary_note requested_name destination duplicate_message
  mkdir -p "$MUSIC_INBOX_QUEUED"
  temporary_note="$(mktemp "$MUSIC_INBOX_QUEUED/.music-inbox-request.XXXXXX")" || {
  ...
  destination="$(music_inbox_safe_note_destination "$MUSIC_INBOX_QUEUED" "$requested_name")"
...
music_inbox_add_request() {
  ...
  if ! music_inbox_submit_request "$url" "$mode" "$playlist" "$create_playlist" "$language" "$formats" >/dev/null; then
...
display dialog (item 1 of argv) with title "Music Inbox could not queue this request" buttons {"OK"} default button "OK" with icon caution
  ...
  music_inbox_add_osascript -e 'on run argv
display notification (item 1 of argv) with title "Music Inbox" subtitle "Request queued"
end run' "${MUSIC_INBOX_SUBMITTED_NOTE:t}" >/dev/null 2>&1 || true
  music_inbox_ui_success "Queued: $MUSIC_INBOX_SUBMITTED_NOTE"
}
